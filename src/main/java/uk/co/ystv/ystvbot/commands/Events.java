...
public class Events extends Command {
	...
	private class EventThread extends TimerTask {
		...
		public void run() {
			...
			try {
				...
				ResultSet result = sql.query("SELECT name, start_date AT TIME ZONE 'GMT' as start_date, end_time, location FROM events WHERE start_date >= now() + interval '60 minutes' AND start_date <= now() + interval '120 minutes' AND is_private = FALSE AND is_cancelled = FALSE ORDER BY start_date ASC");
				while (result.next()) {
	...
	public void onGenericMessage(GenericMessageEvent<PircBotX> event) throws Exception {
		if (event.getMessage().equalsIgnoreCase("!nextevent")) {
			Sql sql = Sql.getInstance();
			ResultSet result = sql.query("SELECT name, start_date AT TIME ZONE 'GMT' as start_date, end_time, location FROM events WHERE start_date > NOW() AND is_private = FALSE AND is_cancelled = FALSE ORDER BY start_date ASC LIMIT 1");
			try {
				result.next();
